...
int main (int argc, char *argv[]) {
  ...
  fdTCP = socket(AF_INET,SOCK_STREAM,0); //TCP socket
	...
	if(errcode != 0){
        gai_strerror(errno);
      printf("Falha no getaddrinfo UDP | %s\n", gai_strerror(errcode));
      exit(1); //error
    }

    errcode = getaddrinfo(NULL,port,&hintsTCP,&resTCP);
	if(errcode != 0){
      printf("Falha no getaddrinfo TCP | %s\n", gai_strerror(errcode));
      exit(1); //error
    }
            ...
            if (!strcmp(tarefa,"ULS")){
                ...
                while ( (next_file = readdir(theFolder)) != NULL ){
                  ...
                  if(strcmp(filepath, fileName) != 0){ // não abre o GROUPS/gid/gid_name.txt
                    ...
                    //Ler o userID do nome do ficheiro
                    for(int c = 0; next_file->d_name[c] != '.'; c++){
                        user_ID[c] = next_file->d_name[c];
                    }
